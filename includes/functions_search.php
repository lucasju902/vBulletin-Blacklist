...
function verify_word_allowed(&$word)
{
	...
	if ($vbulletin->options['fulltextsearch'])
	{
		...
		if (!$badwords)
		{
			$badwords = explode(' ', strtolower($vbulletin->options['badwords']));
		}

		if (preg_match('#^"([^"]+)"$#s', $word, $match))
		{
			// phrase search (in quotes);
			foreach (explode('*', strtolower($match[1])) AS $match_word)
			{
				if (in_array($match_word, $badwords))
				{
					// can't search for this word
					return false;
				}
			}
		}
		...
		{
			// just a single word (not in quotes)
			...
			// if in array, can't search on word
			return (in_array(strtolower($word), $badwords) ? false : true);
		}
