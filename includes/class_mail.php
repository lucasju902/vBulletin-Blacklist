...
*
* @package 		vBulletin
* @version		$Revision: 26206 $
* @date 		$Date: 2008-03-27 06:58:34 -0500 (Thu, 27 Mar 2008) $
...
class vB_Mail
{
	...
	*/
	function start($toemail, $subject, $message, $from = '', $uheaders = '', $username = '')
	{
		...
		if (!$http_host)
		{
			$http_host = substr(md5($message), 12, 6) . '.vb_unknown.unknown';
		}
		$msgid = '<' . gmdate('YmdHs') . '.' . substr(md5($message . microtime()), 0, 12) . '@' . $http_host . '>';
