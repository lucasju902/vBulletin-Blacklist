...
define('CVS_REVISION', '$RCSfile$ - $Revision: 27557 $');
	...
	foreach($phrasearray AS $fieldname => $x)
	{
		...
		foreach($x AS $varname => $y)
		{
			foreach($y AS $phrase)
			{
				...
				$cell[] = '<span class="smallfont">' . nl2br(iif($searchwhere%10 == 0, fetch_highlighted_search_results($vbulletin->GPC['searchstring'], $phrase['text']), htmlspecialchars_uni($phrase['text']))) . '</span>';
